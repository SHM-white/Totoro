...
export default function StartRunPanel({ task, route }) {
  ...
  async function execute() {
    if (pending.current || !task || !route) return;
  ...
  return <section className="start-run" aria-label="开始跑步" aria-busy={busy}>
    ...
    {task && !route && <p className="limit-note">当前任务没有可用路线。</p>}
    <button className="action-button" type="button" disabled={busy || !task || !route} onClick={execute}>
      {busy ? '正在跑步…' : '开始跑步'}
    </button>
