...
const ALLOWED_TRACK_KEYS = ['routeName', 'km', 'usedTime', 'avgSpeed', 'steps'];
...
function sanitizedTrack(track) {
  ...
  return Object.fromEntries(ALLOWED_TRACK_KEYS.map(key => [key, track[key]]));
}
...
export function buildDelayedRunJob({ input, plan, track }, options = {}) {
  ...
  const delayMs = Math.max(1000, Math.round(durationSeconds * 1000));
...
export async function processDelayedRunJob(job, options = {}) {
  ...
  if (!payload?.input || !payload?.plan || !payload?.runStartedAt) {
    throw new Error('延迟任务载荷不完整');
  }
  if (identityFingerprint(payload.input) !== job.data.identityFingerprint) {
    throw new Error('延迟任务身份校验失败');
  }
  const runStartedAt = new Date(payload.runStartedAt);
  if (Number.isNaN(runStartedAt.getTime())) throw new Error('延迟任务开始时间无效');
  const startRunImpl = options.startRunImpl || startRun;
  return startRunImpl(payload.input, { plan: payload.plan, now: runStartedAt });
}
