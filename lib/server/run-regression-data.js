...
export const LOCAL_REGRESSION_TASK = {
  taskId: 'local-regression-paper', name: '本地回归任务', mileage: '3.20', minTime: '10', maxTime: '25', fitDegree: '0.60',
  startDate: '2026-01-01', endDate: '2099-12-31', runTimeRuleList: [{ startTime: '00:00:00', endTime: '23:59:59' }],
  runPointList: [{
    taskId: 'local-regression-paper', pointId: 'local-regression-line', pointName: '本地矩形路线',
...
export function buildRunFixture({ task, route, identity, now = new Date() }) {
  ...
  const token = identity?.token || 'mock-token';
  const version = identity?.version || 'regression-1.0';
  const phoneInfo = identity?.phoneInfo || 'Mock&RouteFixture&Node.js';
