...
export function serviceFromBody(body) {
  return new SunRunService({
    ...
    schoolCode: body.school_code || body.school_id, campusId: body.campus_id,
  });
}

export class SunRunService {
  constructor({ token, stuNumber, schoolCode, schoolId, campusId }, { fetchImpl = fetch } = {}) {
    ...
    this.schoolCode = String(schoolCode || schoolId || '');
  ...
  async getSunrunTask(taskId = '') {
    const tasks = await this.getSunrunTasks();
    const task = taskId ? tasks.find(item => item.taskId === taskId) : tasks[0];
    if (!task) throw new Error(taskId ? '所选任务不存在或已失效' : '当前没有可用跑步任务');
    return task;
  }
